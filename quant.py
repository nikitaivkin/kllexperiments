...
################################################################
class vanillaKLL(QuantProto):
    def __init__(self, s, c=2.0 / 3.0):
        super(vanillaKLL, self).__init__(s, c)

    def update(self, item):
        self.compactors[0].append(item)
        if len(self.compactors[0]) >= self.capacity(0):
            self.compress()
            assert (self.size < self.maxSize)
###############################################################

class vanillaKLLwS(QuantProtoS):
    def __init__(self, s, c=2.0 / 3.0):
        super(vanillaKLLwS, self).__init__(s, c)
...
class BucketM(list):
    def __init__(self, k, bWeight, buckets=None, decisionBit=None):
        ...
        if buckets is not None:
            ...
            # for b in buckets:
            #     temp.extend(b[:])
            #     tempW.extend([b.w]*self.k)
            # temp = np.array(sorted(temp))
            #
            # if self.w % 2 == 1:
            #     self[:] = temp[np.arange(k)*self.w +\
            #                 (self.w+1)/2].tolist()
            # else:
            #     self[:] = temp[np.arange(k) * self.w +\
            #                 (self.w + 2*decisionBit) / 2].tolist()
